...
export class XMLHttpRequestEventTarget extends EventTarget {
  ...
  onprogress: ((this: XMLHttpRequest, ev: Event) => any) | null = null;
...
export class XMLHttpRequest extends XMLHttpRequestEventTarget {
  ...
  #handleResponseEndOfBody() {
    ...
    this.dispatchEvent(new Event("progress", { loaded, total }));
    this.#state = State.DONE;
    this.#sendFlag = false;
    this.dispatchEvent(new Event("readystatechange"));
    this.dispatchEvent(new Event("load", { loaded, total }));
    this.dispatchEvent(new Event("loadend", { loaded, total }));
  }
    ...
    if (!this.#uploadCompleteFlag) {
      ...
      if (this.#uploadListener) {
        this.#upload.dispatchEvent(
          new Event(event, { loaded: 0, total: 0 }),
        );
        this.#upload.dispatchEvent(
          new Event("loadend", { loaded: 0, total: 0 }),
        );
      }
    }
    this.dispatchEvent(new Event(event, { loaded: 0, total: 0 }));
    this.dispatchEvent(new Event("loadend", { loaded: 0, total: 0 }));
  }
  ...
  send(body: BodyInit | null = null): void {
    ...
    this.dispatchEvent(new Event("loadstart", { loaded: 0, total: 0 }));
    this.#upload.dispatchEvent(
      new Event("loadstart", { loaded: 0, total: 0 }),
    );
    ...
    const processRequestEndOfBody = () => {
      ...
      this.#upload.dispatchEvent(
        new Event("progress", { loaded: 0, total: 0 }),
      );
      this.#upload.dispatchEvent(
        new Event("load", {
          loaded: 0,
